-- Add tone and document append settings to round_templates table
ALTER TABLE public.round_templates 
ADD COLUMN tone_settings jsonb DEFAULT '{"aggression_level": "standard", "tone": "professional"}'::jsonb,
ADD COLUMN append_documents jsonb DEFAULT '{"proof_of_address": false, "identity": false, "social_security": false}'::jsonb;

-- Create or replace function to get all round templates (1-12) with default values
...
-- Execute the function to ensure all round templates exist
SELECT public.ensure_round_templates();