...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='customers' AND polname='Users can view their customers'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='customer_users' AND polname='Users can view their own customer mappings'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='credit_rounds' AND polname='Users can view their credit rounds'
  ) THEN
    CREATE POLICY "Users can view their credit rounds"
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='credit_rounds' AND polname='Users can update their credit rounds'
  ) THEN
    CREATE POLICY "Users can update their credit rounds"
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='credit_rounds' AND polname='Users can insert rounds for their customers'
  ) THEN
    CREATE POLICY "Users can insert rounds for their customers"
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='raw_payloads' AND polname='Users can view raw payloads for their rounds'
  ) THEN
    CREATE POLICY "Users can view raw payloads for their rounds"
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='raw_payloads' AND polname='Users can insert raw payloads for their rounds'
  ) THEN
    CREATE POLICY "Users can insert raw payloads for their rounds"
...
DECLARE t TEXT; BEGIN
  ...
  LOOP
    IF NOT EXISTS (
      SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename=t AND polname=concat('Users can view ', t, ' for their rounds')
    ) THEN
      EXECUTE format('CREATE POLICY %L ON public.%I FOR SELECT TO authenticated USING (
        ...
        END
      );', concat('Users can view ', t, ' for their rounds'), t, t, t, t);
    END IF;

    IF t <> 'round_tradeline_history' THEN
      IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename=t AND polname=concat('Users can insert ', t, ' for their rounds')
      ) THEN
        EXECUTE format('CREATE POLICY %L ON public.%I FOR INSERT TO authenticated WITH CHECK (
          EXISTS (
            SELECT 1 FROM public.credit_rounds r
            JOIN public.customer_users cu ON cu.customer_id = r.customer_id
            WHERE r.id = %I.credit_round_id AND cu.user_id = auth.uid() AND r.deleted_at IS NULL
      ...
      IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename=t AND polname=concat('Users can update ', t, ' for their rounds')
      ) THEN
        EXECUTE format('CREATE POLICY %L ON public.%I FOR UPDATE TO authenticated USING (
          EXISTS (
            SELECT 1 FROM public.credit_rounds r
            JOIN public.customer_users cu ON cu.customer_id = r.customer_id
            WHERE r.id = %I.credit_round_id AND cu.user_id = auth.uid() AND r.deleted_at IS NULL
