...
-- Policies: users can view/insert their own run events; superadmins full access
CREATE POLICY IF NOT EXISTS "Users can view their own import events"
...
CREATE POLICY IF NOT EXISTS "Users can insert their own import events"
ON public.smart_credit_import_events
FOR INSERT
WITH CHECK (
  auth.uid() = (
...
-- Optional: allow superadmins to manage events
CREATE POLICY IF NOT EXISTS "Superadmins can manage import events"
ON public.smart_credit_import_events
AS RESTRICTIVE
FOR ALL
...
-- Function to clean up expired events (can be called from maintenance jobs)
