-- Idempotent rebuild for Smart Credit objects (safe re-run)
...
CREATE INDEX IF NOT EXISTS idx_sci_user_posted_at ON public.smart_credit_items (user_id, posted_at DESC);
CREATE INDEX IF NOT EXISTS idx_sci_run_id ON public.smart_credit_items (run_id);
CREATE INDEX IF NOT EXISTS idx_scie_run_ts ON public.smart_credit_import_events (run_id, ts);

-- Unique constraint for deterministic upserts
DO $$ BEGIN
  IF NOT EXISTS (
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    CREATE POLICY "Users can view their own imports" ON public.smart_credit_imports FOR SELECT USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (
    ...
    CREATE POLICY "Users can insert their own imports" ON public.smart_credit_imports FOR INSERT WITH CHECK (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (
    ...
    CREATE POLICY "Users can update their own imports" ON public.smart_credit_imports FOR UPDATE USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (
    ...
    CREATE POLICY "Users can delete their own imports" ON public.smart_credit_imports FOR DELETE USING (auth.uid() = user_id);
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    CREATE POLICY "Users can view their own items" ON public.smart_credit_items FOR SELECT USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (
    ...
    CREATE POLICY "Users can insert their own items" ON public.smart_credit_items FOR INSERT WITH CHECK (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (
    ...
    CREATE POLICY "Users can update their own items" ON public.smart_credit_items FOR UPDATE USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (
    ...
    CREATE POLICY "Users can delete their own items" ON public.smart_credit_items FOR DELETE USING (auth.uid() = user_id);
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    CREATE POLICY "Users can view their run events" ON public.smart_credit_import_events FOR SELECT USING (
      EXISTS (SELECT 1 FROM public.smart_credit_imports i WHERE i.run_id = smart_credit_import_events.run_id AND i.user_id = auth.uid())
    );
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    CREATE POLICY "Users can view their own smart credit credentials" ON public.smart_credit_credentials FOR SELECT USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (
    ...
    CREATE POLICY "Users can upsert their own smart credit credentials" ON public.smart_credit_credentials FOR INSERT WITH CHECK (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (
    ...
    CREATE POLICY "Users can update their own smart credit credentials" ON public.smart_credit_credentials FOR UPDATE USING (auth.uid() = user_id);
  END IF;
END $$;
